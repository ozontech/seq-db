...
func MergeIndexes(indexes []*Index) *Index {
	...
	fieldsCount := 0
	docsSize := uint64(0)
	iterators := make([]indexIterator, 0, len(indexes))
...
func mergeIDs(dst *Index, orig []indexIterator) []seq.ID {
	doubles := []seq.ID{}
	iterators := append([]indexIterator{}, orig...) // make copy
	for len(iterators) > 0 {
		// try select first
		selected := []int{0}
...
func mergeTokens(dst *Index, orig []indexIterator) {
...
func mergePositions(dst *Index, orig []indexIterator) {
	iterators := append([]indexIterator{}, orig...) // make copy
	for len(iterators) > 0 {
		// try select first
		selected := []int{0}
