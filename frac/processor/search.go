...
func iterateEvalTree(
	...
	if bn, ok := evalTree.(node.BatchedNode); ok {
		return iterateEvalTreeBatched(ctx, params, idsIndex, bn, aggs, sw)
	}
...
func iterateEvalTreeBatched(
	...
	for i := 0; ; i++ {
		...
		timerEval.Start()
		lidBatch := evalTree.NextBatch(0, 4096)
		timerEval.Stop()
