...
func TestSingleSourceCountAggregator(t *testing.T) {
	...
	for _, id := range searchDocs {
		if err := agg.Next(node.NewLIDDesc(id)); err != nil {
			t.Fatal(err)
		}
	}
...
func BenchmarkAggDeep(b *testing.B) {
	...
	for _, s := range sizes {
		b.Run(fmt.Sprintf("size=%d", s), func(b *testing.B) {
			...
			for b.Loop() {
				for _, v := range vals {
					if err := n.Next(node.NewLIDDesc(v)); err != nil {
						b.Fatal(err)
					}
				}
			}
...
func TestTwoSourceAggregator(t *testing.T) {
	...
	field := &MockNode{
		Pairs: []IDSourcePair{
			{LID: node.NewLIDDesc(1), Source: 0},
			{LID: node.NewLIDDesc(2), Source: 1},
		},
	}
	groupBy := &MockNode{
		Pairs: []IDSourcePair{
	...
	r.NoError(aggregator.Next(node.NewLIDDesc(1)))
	r.NoError(aggregator.Next(node.NewLIDDesc(2)))
