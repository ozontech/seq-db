...
func NewBlockPackerWithThreshold(lidsBitmapThreshold int) *BlockPacker {
	p := NewBlockPacker()
	if lidsBitmapThreshold != 0 {
		p.lidsBitmapThreshold = lidsBitmapThreshold
	}
	return p
}
...
func (p *BlockPacker) writeAsBitmap(dst []byte, lids []uint32) []byte {
	p.bm.Clear()
	p.bm.AddMany(lids)
	p.bm.RunOptimize()
...
func (b *UnpackedBlock) GetSizeBytes() int {
	const uint32Size = 4
	return int(uint32Size*len(b.LIDs) + uint32Size*len(b.Offsets))
}
