...
func NewIndexBlockHeader(pos int64, ext1, ext2 uint64, payloadLen, rawLen int, codec Codec) IndexBlockHeader {
	header := NewEmptyIndexBlockHeader()
	header.SetExt1(ext1)
	header.SetExt2(ext2)
	header.SetLen(uint32(payloadLen))
	header.SetRawLen(uint32(rawLen))
