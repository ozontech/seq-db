...
func NewSourcedNodeIterator(sourced node.Sourced, index frac.TokenIndex, tids []uint32, limit int, reverse bool) *SourcedNodeIterator {
...
func (s *SourcedNodeIterator) ValueBySource(source uint32) string {
	...
	if s.countBySource[source] < useCacheThreshold {
		return string(s.index.GetValByTID(s.tids[source]))
	}
	...
	val = string(s.index.GetValByTID(s.tids[source]))
