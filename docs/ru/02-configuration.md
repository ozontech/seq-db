...
Настройки фоновой компакции, которая объединяет небольшие запечатанные фракции в более крупные, чтобы поиск оставался быстрым.

| Параметр | Тип | Значение по умолчанию | Описание |
|----------|-----|----------------------|-----------|
| `compaction.enabled` | bool | `false` | Главный переключатель фоновой компакции |
| `compaction.workers` | int | runtime.GOMAXPROCS | Количество воркеров-исполнителей, параллельно выполняющих слияния |
| `compaction.time_window` | Duration | `1h` | Ширина временной корзины. Объединяются только фракции, время создания которых попадает в одну корзину |
| `compaction.tick_interval` | Duration | `1s` | Как часто планировщик просыпается, чтобы выбрать задачу компакции |
| `compaction.stcs.merge_trigger` | int | `4` | Минимальное количество фракций в size-корзине, необходимое для запуска слияния |
| `compaction.stcs.merge_fan_in` | int | `32` | Максимальное количество фракций, объединяемых из одной корзины за итерацию |
| `compaction.stcs.merge_fan_out_size` | Bytes | `512MiB` | Верхняя граница суммарного размера индекса входных фракций одного слияния, ограничивающая размер результирующей фракции |
| `compaction.stcs.bucket_lowerbound` | float64 | `0.5` | Нижний коэффициент размера для попадания в корзину (размер фракции ≥ `bucket_lowerbound × avg(корзины)`) |
| `compaction.stcs.bucket_upperbound` | float64 | `1.5` | Верхний коэффициент размера для попадания в корзину (размер фракции ≤ `bucket_upperbound × avg(корзины)`) |
