...
func (n *nodeOrBatched) NextBatch(need int) LIDBatch {
	if n.desc {
...
func (n *nodeOrBatched) NextBatchGeq(need int, minLID LID) LIDBatch {
	for {
...
func (n *nodeOrBatchedMulti) NextBatch(need int) LIDBatch {
	if n.desc {
		return n.NextBatchGeq(need, NewDescZeroLID())
	}
	return n.NextBatchGeq(need, NewAscZeroLID())
...
func (n *nodeOrBatchedMulti) NextBatchGeq(need int, minLID LID) LIDBatch {
	for {
		active := 0
		for i := range n.children {
			if n.batches[i].IsEmpty() && !n.done[i] {
				n.batches[i] = n.children[i].NextBatchGeq(need, minLID)
