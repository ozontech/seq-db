...
func TestColumnAgg_MatchesOrTree_AscDocsOrder(t *testing.T) {
	sources := [][]uint32{
		{1, 3, 5},
		{2, 4, 6},
	}
	...
	tree := BuildORTreeAgg([]Node{
		NewStatic(sources[0], true),
		NewStatic(sources[1], true),
	})
	cursors := make([]BatchedNode, len(sources))
	for i, src := range sources {
	...
	// Asc docs order: consume from high LID to low.
...
func TestColumnAgg_NextSourcedGeq(t *testing.T) {
	...
	id, source := column.NextSourcedGeq(NewDescLID(3))
	assert.Equal(t, uint32(3), id.Unpack())
	assert.Equal(t, uint32(1), source)

	id, source = column.NextSourcedGeq(NewDescLID(6))
...
func TestColumnAgg_NextSourcedGeq_AscDocsOrder(t *testing.T) {
	// desc=false: docs order asc, raw LIDs decrease
	sources := [][]uint32{
		{1, 2, 3, 5, 8, 15, 19},
		{2, 3, 4, 6, 8, 14, 20},
	}
	...
	id, source = column.NextSourcedGeq(NewAscLID(8))
	assert.Equal(t, uint32(8), id.Unpack())
	assert.Equal(t, uint32(1), source)

	id, source = column.NextSourcedGeq(NewAscLID(4))
	assert.Equal(t, uint32(4), id.Unpack())
	assert.Equal(t, uint32(1), source)

	id, source = column.NextSourcedGeq(NewAscLID(1))
	assert.Equal(t, uint32(1), id.Unpack())
	assert.Equal(t, uint32(0), source)

	id, _ = column.NextSourcedGeq(NewAscLID(1))
