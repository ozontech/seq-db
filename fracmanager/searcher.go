...
func (s *Searcher) SearchDocs(ctx context.Context, fracs []frac.Fraction, params searcher.Params) (*seq.QPR, error) {
...
func (s *Searcher) prepareFracs(fracs List, params searcher.Params) (List, error) {
	fracs = fracs.FilterInRange(params.From, params.To)
	if s.cfg.MaxFractionHits > 0 && len(fracs) > s.cfg.MaxFractionHits {
		return nil, fmt.Errorf(
			"error: %w: (%d > %d), try decreasing query time range",
			ErrTooManyFractionsHit,
			len(fracs),
...
func (s *Searcher) searchDocsAsync(ctx context.Context, fracs []frac.Fraction, params searcher.Params) ([]*seq.QPR, error) {
...
func (s *Searcher) fracSearch(ctx context.Context, params searcher.Params, f frac.Fraction) (_ *seq.QPR, err error) {
	defer func() {
