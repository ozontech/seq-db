...
func newMetaToken(k, v string) MetaToken {
...
func TestKeywordTokenizerSimple1(t *testing.T) {
	...
	value := []byte("woRld")
	expected := []MetaToken{newMetaToken("message", "woRld")}
...
func TestKeywordTokenizerCaseSensitive(t *testing.T) {
	...
	assert.Equal(t, []MetaToken{newMetaToken("message", "hello world")}, tokens)
}
...
func TestKeywordTokenizerPartialIndexing(t *testing.T) {
	...
	assert.Equal(t, []MetaToken{newMetaToken("message", value[:maxSize])}, tokens)
